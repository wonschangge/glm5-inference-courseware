...
> **不计入覆盖率的引用**：`source.md` 共 21 个引用块，其中 **18 个**来自上表六个文件。
...
4. **两版切块** —— `divide_to_patches` 的光栅序列表 vs `patchify` 的张量重排（6×6 图对照）
5. **★ 合并前的重排** —— `patchify` 的 8 维 reshape + 一次 `permute(0,2,5,3,6,1,4,7)`
6. **★ 算一遍 336×336** —— 24×24=576，576/4=**144**（验收点 1）
7. **group_images_by_shape** —— 为什么不同尺寸的 grid 不能堆在一起 + CPU 不分组（PR #38157）
8. **视频 = 同一套变换 + 采样** —— `arange(0, N, N/n)` 等间隔抽帧、帧索引与 `sampled_fps`
9. **采样接口的三条规则** —— 三级默认、`fps` 与 `num_frames` 互斥、硬边界
10. **★ 收束与 temporal patch** —— 帧数必须是 `temporal_patch_size` 的整数倍、补最后一帧（验收点 2）；
    形状总表 + 三个可复算的数字 + 两个练习 + 下一课指路
...
| 引用块数 | 8–14 | **21**（域内 18 + 域外 3） | 六个文件各 ≥2 块；超出部分用于支撑两个验收点（带 `merge_size` 的切块与占位符计数只存在于 L2-06 的三个文件里） |
