...
★ 所有 ```python 引用块都从上游源文件**切片**得到（slice_），绝不手打重写。
用法：  python3 _data/recon/gen_l303_source.py
"""
import os

SRC = "/data/WORKSPACE/transformer-project/upstream-transformers/src/transformers/models/glm5_next/modeling_glm5_next.py"
OUT = "/data/WORKSPACE/transformer-project/courseware/L3-backbone/L3-03/source.md"
REL = "models/glm5_next/modeling_glm5_next.py"

_LINES = open(SRC, encoding="utf-8").read().split("\n")
assert _LINES[145].startswith("class Glm5NextTextTopkRouter"), _LINES[145]


def slice_(a, b):
    """源文件第 a..b 行（1-indexed，含两端）的逐字副本。"""
    return "\n".join(_LINES[a - 1:b])
...
print("wrote", OUT, sum(len(x) for x in P), "chars")
print("python blocks:", "".join(P).count("```python"))
