...
/* ------------------------------------------------- 4 无权重版用在哪 */
{
  ...
  build(root, tl) {
    ...
    const STATS = [
      ...
      { cc: 3, k: '省下 1,474,560', s: '若加权重：90 x 16384 个参数（仅占 0.00047%）' },
      { cc: 0, k: 'x100 后系数不变', s: '实测：输入整体放大 100 倍，pre/post/comb 逐位不变' },
    ];
    const srow = U.el('div', { class: 'row gap12', style: 'width:100%;align-items:stretch' });
    const stats = STATS.map(st => {
    ...
    tl.at(14800, () => {
      stats[2].classList.remove('ac'); stats[3].classList.add('ac');
      msg.innerHTML = '实测：输入整体放大 <em>100 倍</em>，pre/post/comb <em>逐位不变</em> —— 归一化保证的是「系数与尺度无关」';
    });
    tl.at(17400, () => {
      stats[3].classList.remove('ac');
      msg.innerHTML = '★ 而它的输出只喂给一个线性映射：<em>F(w ⊙ x) = (F ∘ diag(w))(x)</em> 还是线性映射 —— 这里的 weight 可以被 fn 吸收，是多余的';
    });
  },
},
...
/* ------------------------------------------------- 6 门控解决什么 */
{
  ...
  build(root, tl) {
    ...
    viz.appendChild(U.el('div', { style: 'width:100%;height:10px' }));
    ...
    const rc = (e) => U.rectIn(e, U.q('#visual'));
    const gA = rc(gateBox), kA = rc(kdaBox), oB = rc(on);
    const yBar = oB.y - 16;
    FLOW.path('g', [[gA.cx, gA.y + gA.h], [gA.cx, yBar], [oB.x + oB.w * 0.30, yBar], [oB.x + oB.w * 0.30, oB.y]]);
    FLOW.path('k', [[kA.cx, kA.y + kA.h], [kA.cx, yBar], [oB.x + oB.w * 0.70, yBar], [oB.x + oB.w * 0.70, oB.y]]);
...
/* ------------------------------------------------- 8 分布与代价 */
{
  ...
  build(root, tl) {
    ...
    const row = U.el('div', { class: 'row gap14', style: 'width:100%;align-items:stretch' });
    const bars = W.bars([
      { label: 'TextRMSNorm', value: 113, valueText: '113', cc: 0 },
